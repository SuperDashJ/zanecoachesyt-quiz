const SITE_URL =
  ...
  (process.env.VERCEL_PROJECT_PRODUCTION_URL
    ...
    : "https://zanecoachesyt.com");

const IMAGE_BY_LABEL = {
  "Unlocking monetization": "/quiz-assets/art/creator-unlock.png",
  "Making money from content": "/quiz-assets/art/creator-revenue.png",
  "Growing creator income": "/quiz-assets/art/creator-money.png",
  "I spend too much time on analytics.": "/quiz-assets/art/creator-analytics.png",
  "I feel like the algorithm is against me.": "/quiz-assets/art/creator-algorithm.png",
  "I overthink and don't post enough.": "/quiz-assets/art/creator-overthinking.png",
  "Getting people to click": "/quiz-assets/art/creator-clicks.png",
  "Keeping people watching": "/quiz-assets/art/creator-retention.png",
  "Publishing enough to learn": "/quiz-assets/art/creator-publishing.png",
...
const CHANNEL_URL = "https://www.youtube.com/@ZaneCoachesYoutube";
const HEADSHOT_URL =
...
function blockerPhrase(blocker) {
  switch (blocker) {
    case "I spend too much time on analytics.":
      return "analytics are";
    case "I feel like the algorithm is against me.":
      return "algorithm frustration is";
    case "I overthink and don't post enough.":
      return "overthinking is";
    default:
      return `${blocker} is`;
  }
}
...
function cutFriction(blocker) {
  switch (blocker) {
    case "I spend too much time on analytics.":
      return "Check analytics once, write down one lesson, then close it.";
    case "I feel like the algorithm is against me.":
      return "Change one variable on the next upload instead of blaming the whole algorithm.";
    case "I overthink and don't post enough.":
      return "Publish the smallest finished version before adding more ideas.";
    default:
...
function smallAction(priority, time) {
  switch (priority) {
    case "Unlocking monetization":
      return `Use ${time} to move one upload closer to monetization.`;
    case "Making money from content":
      return `Use ${time} for one creator money move: improve the offer, video idea, or next step.`;
    case "Growing creator income":
      return `Use ${time} to make the viewer's next step clearer.`;
    default:
...
function priorityDailyActions(priority) {
  switch (priority) {
    case "Unlocking monetization":
      return ["Improve one video idea.", "Make one audience-building move."];
    case "Making money from content":
      return ["Sharpen one creator offer.", "Improve one call to action."];
    case "Growing creator income":
      return ["Clarify the next step for viewers.", "Improve one content promise."];
    default:
...
function blockerDailyAction(blocker) {
  switch (blocker) {
    case "I spend too much time on analytics.":
      return "Check analytics once, then close it.";
    case "I feel like the algorithm is against me.":
      return "Test one change on the next upload.";
    case "I overthink and don't post enough.":
      return "Post the smallest finished version.";
    default:
...
<html lang="en">
            ...
            <tr>
              <td style="padding:22px 34px 34px;">
                ...
                <div style="margin:18px 0 0; padding:18px 20px; border-radius:18px; background:#111825; text-align:center;">
                  <p style="margin:0 0 12px; color:#ffffff; font-size:16px; line-height:1.5; font-weight:800;">Click for more coaching videos 👉</p>
  ...
  const text = [
    ...
    "",
    "Click for more coaching videos 👉",
    CHANNEL_URL,
    "",
    "- Zane"
  ].join("\n");
...
export async function sendReportEmail({ email, profile }) {
  ...
  const from =
    !configuredFrom || configuredFrom.includes("onboarding@resend.dev")
      ? "ZaneCoachesYT <results@zanecoachesyt.com>"
      : configuredFrom;
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    headers: {
      ...
      "User-Agent": "zanecoachesyt-quiz/1.0"
    },
    body: JSON.stringify({
      from,
      to: [email],
