...
async function createInitialBranch(repository, token, branch, defaultBranch) {
  const fileResponse = await githubRequest(
    `https://api.github.com/repos/${repository}/contents/.gitkeep`,
    {
      method: "PUT",
      body: JSON.stringify({
        message: "Initialize lead storage",
        content: toBase64(""),
...
async function ensureBranch(repository, token, branch) {
  ...
  if (branchResponse.status === 409) {
    const body = await branchResponse.text();

    if (body.includes("Git Repository is empty")) {
      await createInitialBranch(repository, token, branch, defaultBranch);
      return;
    }
...
async function saveLeadToGitHub(record) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    const payload = {
      ...
      branch,
      committer: {
        name: process.env.GITHUB_STORAGE_COMMITTER_NAME || "ZaneCoachesYT Leads",
        email:
